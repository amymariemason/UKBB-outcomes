...
gen gest_later= n_4041_`m'_0
recode gest_later -2=0 -3=. -1=.

replace gest_change = 1 if dm_gdmonly_sr_bl==1 & gest_later==0

drop gest_later
}

noi di "number who said yes, then later No to Qu: gestational diabetes only"
noi summ gest_change
noi list n_eid if gest_change==1

replace dm_gdmonly_sr_bl=0 if gest_change==1
drop n_4041* gest*
...
forval m=1(1)`survey' {
gen agediag_sr_bl`m'=n_2976_`m'_0
replace agediag_sr_bl`m' = . if agediag_sr_bl`m'<0
}

noi di "age of diagnosis of diabetes if diffs by > 5 years in subsequent reports"
egen min_all= rowmin (dm_agediag_sr_bl0 agediag_sr_bl*)
egen max_all= rowmax (dm_agediag_sr_bl0 agediag_sr_bl*)
egen med_all= rowmedian (dm_agediag_sr_bl0 agediag_sr_bl*)
egen mean_all = rowmean (dm_agediag_sr_bl0 agediag_sr_bl*)
gen morethan5=1 if max_all-min_all>5  & max_all!=.
noi summ morethan5
noi list dm_agediag_sr_bl agediag_sr_bl* med_all mean_all if morethan5==1

* As a judgement call: take median value if multiple reported ages

gen dm_agediag_sr_bl = med_all
drop  agediag_sr_bl* *_all morethan5  
noi di "age of diagnosis of diabetes original survey"
noi summ dm_agediag_sr_bl0

noi di "age of diagnosis of diabetes encorporating all surveys (median value)"
noi summ dm_agediag_sr_bl
...
noi summ dm_drug_ins_men_org if sex==1

noi di "encorporating all surveys: male current insulin intake"
noi summ dm_drug_ins_men
...
* loop over each column in surveys
	ds n_20002_`m'_*
	macro drop _cols
	local cols:  word count `r(varlist)'
	local cols = `cols'-1

* record age value matching 1220	
	forval i=0(1)`cols' {
	replace temp_id_`m'=1 if n_20002_`m'_`i'==1220
	replace temp_age_`m'= n_20009_`m'_`i' if n_20002_`m'_`i'==1220 & n_20009_`m'_`i' >0
	
	}

}

egen dm_alldm_ni_bl= anymatch(temp_id*), values(1) 
...
summ temp_id_0 if temp_id_0==1
noi di _N "people with Diabetes non-specified on first survey"

summ dm_alldm_ni_bl if  dm_alldm_ni_bl ==1
noi di _N "people with Diabetes non-specified on all surveys"

noi tab temp_id_0 dm_alldm_ni_bl 
...
* record age value matching 1221	
	forval i=0(1)`cols' {
	replace temp_id_`m'=1 if n_20002_`m'_`i'==1221
	replace temp_age_`m'= n_20009_`m'_`i' if n_20002_`m'_`i'==1221 & n_20009_`m'_`i' >0
	}

}

egen dm_gdm_ni_bl= anymatch(temp_id*), values(1) 
...
summ temp_id_0 if temp_id_0 ==1
noi di _N "people with Gestational diabetes on first survey"

summ dm_gdm_ni_bl if dm_gdm_ni_bl ==1
noi di _N " people with Gestational diabetes on later survey"
...
* record age value matching 1222	
	forval i=0(1)`cols' {
	replace temp_id_`m'=1 if n_20002_`m'_`i'==1222
	replace temp_age_`m'= n_20009_`m'_`i' if n_20002_`m'_`i'==1222 & n_20009_`m'_`i' >0
	}
...
egen dm_t1dm_ni_bl= anymatch(temp_id*), values(1) 
...
summ temp_id_0 if temp_id_0==1
noi di _N "people with T1 diabetes on first survey"


summ dm_t1dm_ni_bl if  dm_t1dm_ni_bl ==1
noi di _N "people with T1 diabetes on later survey"
...
* record age value matching 1223	
	forval i=0(1)`cols' {
	replace temp_id_`m'=1 if n_20002_`m'_`i'==1223
	replace temp_age_`m'= n_20009_`m'_`i' if n_20002_`m'_`i'==1223 & n_20009_`m'_`i' >0
	}

}

egen dm_t2dm_ni_bl=anymatch(temp_id*), values(1) 
...
summ temp_id_0 if temp_id_0==1
noi di _N "people with T2 diabetes on first survey"


summ dm_t2dm_ni_bl if  dm_t2dm_ni_bl ==1
noi di _N "people with T2 diabetes on later survey"
...
* find matching people
	forval i=0(1)`cols' {
	replace dm_drug_ins_ni_bl=1 if n_20003_`m'_`i'==1140883066
	}
}
...
* find matching people
	forval i=0(1)`cols' {
	replace dm_drug_metf_ni_bl=1 if n_20003_`m'_`i'==1140884600 
	replace dm_drug_metf_ni_bl=1 if n_20003_`m'_`i'==1140874686
	replace dm_drug_metf_ni_bl=1 if n_20003_`m'_`i'==1141189090
	}
}
...
* find matching people
	forval i=0(1)`cols' {
	replace dm_drug_su_ni_bl=1 if n_20003_`m'_`i'==1140874718 
	replace dm_drug_su_ni_bl=1 if n_20003_`m'_`i'==1140874744
	replace dm_drug_su_ni_bl=1 if n_20003_`m'_`i'==1140874746
	replace dm_drug_su_ni_bl=1 if n_20003_`m'_`i'==1141152590 
	replace dm_drug_su_ni_bl=1 if n_20003_`m'_`i'==1141156984
	replace dm_drug_su_ni_bl=1 if n_20003_`m'_`i'==1140874646
	replace dm_drug_su_ni_bl=1 if n_20003_`m'_`i'==1141157284 
	replace dm_drug_su_ni_bl=1 if n_20003_`m'_`i'==1140874652
	replace dm_drug_su_ni_bl=1 if n_20003_`m'_`i'==1140874674
	replace dm_drug_su_ni_bl=1 if n_20003_`m'_`i'==1140874728 
	}
}
...
* find matching people
	forval i=0(1)`cols' {
replace dm_drug_other_oad_ni_bl=1 if n_20003_`m'_`i'==1140868902 
replace dm_drug_other_oad_ni_bl=1 if n_20003_`m'_`i'==1140868908
replace dm_drug_other_oad_ni_bl=1 if n_20003_`m'_`i'==1140857508
}
}
...
* find matching people
	forval i=0(1)`cols' {
replace dm_drug_meglit_ni_bl=1 if n_20003_`m'_`i'==1141173882 
replace dm_drug_meglit_ni_bl=1 if n_20003_`m'_`i'==1141173786
replace dm_drug_meglit_ni_bl=1 if n_20003_`m'_`i'==1141168660
}
}
...
* find matching people
	forval i=0(1)`cols' {
replace dm_drug_glitaz_ni_bl=1 if n_20003_`m'_`i'==1141171646 
replace dm_drug_glitaz_ni_bl=1 if n_20003_`m'_`i'==1141171652
replace dm_drug_glitaz_ni_bl=1 if n_20003_`m'_`i'==1141153254
replace dm_drug_glitaz_ni_bl=1 if n_20003_`m'_`i'==1141177600
replace dm_drug_glitaz_ni_bl=1 if n_20003_`m'_`i'==1141177606
}
}
