...
local selfreports 20002 20004 6150 6152 6153 6177
...
if `Pre_wanted'==1{
	* check self report age files exist if pre/primary care data wanted
	local selfreports 20008 20010 3627 3894 2966 4056 4012 4022 3786 3992 3761 33
	foreach srnum of local selfreports{
	capture confirm variable n_`srnum'_0_0
	if _rc!=0 {
		noi di  as error "ERROR: variable `srnum' missing in Step 3b"
			error 498
		}
	}
	capture confirm variable ts_53_0_0
	if _rc!=0 {
		noi di  as error "ERROR: variable ts_53 missing in Step 3b"
			error 498
	}
					...
					if "`code'"!="NO CODES"{
						...
						*7	Blood clot in the lung: 4022
						if regexm("7","`code'"){
							...
							gen tempage = n_4022_`m'_0 if (n_6152_`m'_`i'==7) & n_4022_`m'_0 >=0
